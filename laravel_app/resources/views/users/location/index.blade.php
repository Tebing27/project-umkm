<x-layouts.guest :title="translate('Lokasi UMKM - UMKM Sasuma')" :header-title="translate('Lokasi UMKM')" :header-subtitle="translate('Tambahkan lokasi baru')">
    ...
    <div x-data="locationHybrid()" x-init="init()" class="relative">
        @include('users.location.partials._header')
        @include('users.location.partials._alerts')
        ...
        <form action="{{ url('/users/lokasi/store') }}" method="POST" class="space-y-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                @include('users.location.partials._map-preview')
                @include('users.location.partials._address-form')
            </div>
        </form>

        @include('users.location.partials._mobile-modal')
    </div>

    @push('scripts')
        @include('users.location.partials._scripts')
    @endpush
</x-layouts.guest>
