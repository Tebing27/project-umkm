                ...
                <div class="space-y-6 flex flex-col justify-center">
                    ...
                    {{-- 1. WILAYAH --}}
                    <div class="order-1">
                        <label class="block text-base font-semibold text-slate-900 mb-2">
                        ...
                        <div class="relative" x-data="{
                            ...
                            <x-ui.button type="button" @click="open = !open" variant="ghost"
                                class="group w-full cursor-pointer bg-gray-50 rounded-lg border border-gray-300 hover:bg-gray-50 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 py-2.5 px-4 flex items-center justify-between transition-all">
                            ...
                            <div x-show="open" x-transition.origin.top x-cloak
                                ...
                                <div @click="select('', '{{ translate('Pilih Wilayah') }}')"
                                    class="px-5 py-3 text-sm md:text-base font-medium text-slate-600 hover:bg-gray-50 cursor-pointer flex items-center justify-between transition-colors border-b border-gray-50">
                                    <span>{{ translate('Pilih Wilayah') }}</span>
                                </div>
                                @foreach ($regions as $region)
                                    <div @click="select('{{ $region->id }}', '{{ $region->name }}')"
                                        class="px-5 py-3 text-sm md:text-base font-medium cursor-pointer flex items-center justify-between transition-colors hover:bg-gray-50"
                    ...
                    <div class="order-2 lg:order-4">
                        <label class="block mb-2 text-base font-semibold text-slate-900">{{ translate('Alamat Usaha') }}</label>
                        <div class="relative">
                    ...
                    <div class="order-3 lg:order-2">
                        <label class="block mb-2 text-base font-semibold text-slate-900">{{ translate('Latitude') }}</label>
                    ...
                    <div class="order-4 lg:order-3">
                        <label class="block mb-2 text-base font-semibold text-slate-900">{{ translate('Longitude') }}</label>
                        {{-- [UBAH] Tambahkan click event untuk alert --}}
                        <div @click="if(isMobile) triggerInstructionAlert()">
