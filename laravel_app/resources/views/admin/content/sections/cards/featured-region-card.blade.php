...
<form x-ref="form" action="{{ route('admin.regions.update_featured_shop', $region->id) }}" method="POST" @submit.prevent="submitForm">
    @csrf @method('PUT')
    @include('admin.content.sections.cards.partials.featured-region-header')
    <div class="flex gap-2 items-center">
            @include('admin.content.sections.cards.partials.featured-region-dropdown')
            @include('admin.content.sections.cards.partials.featured-region-submit')
    </div>
</form>
</div>
