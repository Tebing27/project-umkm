    ...
    window.initBusinessTypes = function(config) {
        return {
            ...
            openAdd() {
                ...
                this.fallbackIconHtml = this.getFallbackHtml('map-pin');
            ...
            openEdit(item) {
                ...
                this.fallbackIconHtml = this.getFallbackHtml(item.fallback_icon);
            ...
            getFallbackHtml(iconName) {
                const map = {
                    'map-pin-food': 'icon-food',
                    'map-pin-fashion': 'icon-fashion',
                    'map-pin-work': 'icon-work',
                    'shop': 'icon-kelontong',
                    'plants': 'icon-agribisnis',
                    'hand-craft': 'icon-kerajinan',
                    'map-pin': 'icon-grid'
                };
                const id = map[iconName] || 'icon-grid';
...
<div x-data="initBusinessTypes(window.businessTypesConfig)">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        @foreach ($businessTypes as $item)
            <div class="group relative bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1 p-5">
                ...
                <div class="flex justify-between items-start mb-4">
                     ...
                     <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button @click="openEdit({ 
                                ...
                                fallback_icon: '{{ $item->fallback_marker_icon }}'
                            })" 
                            class="p-1.5 bg-gray-100 rounded-lg text-slate-500 hover:text-primary hover:bg-primary/10 transition-colors"
                        ...
                        <form action="{{ route('admin.contents.destroy', $item->id) }}" method="POST" 
                            ...
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="p-1.5 bg-gray-100 rounded-lg text-slate-500 hover:text-red-500 hover:bg-red-50 transition-colors"
                    ...
                    <div class="flex-1 flex flex-col items-center gap-2">
                        <div class="w-14 h-14 rounded-xl bg-blue-50/50 border border-blue-100 flex items-center justify-center text-blue-600 relative overflow-hidden group/marker">
                            @if(isset($item->icon_url) && $item->icon_url)
                                <img loading="lazy" src="{{ asset('storage/' . $item->icon_url) }}" alt="Marker {{ $item->value }}" class="w-8 h-8 object-contain drop-shadow-sm">
                            @else
                                <x-dynamic-component :component="'icons.' . $item->fallback_marker_icon" class="w-8 h-8" />
                            @endif
                        </div>
                        <span class="text-xs uppercase tracking-wider font-semibold text-slate-400">Peta</span>
                    </div>
                    ...
                    <div class="flex-1 flex flex-col items-center gap-2">
                        <div class="w-14 h-14 rounded-xl bg-purple-50/50 border border-purple-100 flex items-center justify-center relative overflow-hidden p-2 group/logo">
                            @if(isset($item->logo_url) && $item->logo_url)
                                <img loading="lazy" src="{{ asset('storage/' . $item->logo_url) }}" class="w-full h-full object-contain">
                            @else
                                <img loading="lazy" src="{{ asset('images/' . $item->fallback_logo) }}" alt="Logo Standard {{ $item->value }}" class="w-full h-full object-contain opacity-80" title="Default System Logo">
                            @endif
                        </div>
                        <span class="text-xs uppercase tracking-wider font-semibold text-slate-400">Toko</span>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="flex flex-col gap-3">
                                <div class="w-full h-32 rounded-lg bg-gray-50 flex items-center justify-center border border-gray-200 overflow-hidden relative group">
                                    <template x-if="markerPreview">
                                        <img loading="lazy" :src="markerPreview" alt="Marker Preview" class="w-12 h-12 object-contain">
                                    </template>
                                    <template x-if="!markerPreview">
                                        <div class="flex flex-col items-center gap-1 text-slate-300">
                                            <div class="w-8 h-8 text-slate-300" x-html="fallbackIconHtml"></div>
                                ...
                                <span class="text-xs text-slate-400">SVG Only. Max 1MB.</span>
                            </div>
                        </div>
                             ...
                             <div class="flex flex-col gap-3">
                                 <div class="w-full h-32 rounded-lg bg-gray-50 flex items-center justify-center border border-gray-200 overflow-hidden relative group">
                                     <template x-if="logoPreview">
                                         <img loading="lazy" :src="logoPreview" alt="Logo Preview" class="w-12 h-12 object-contain">
                                     </template>
                                     <template x-if="!logoPreview">
                                         <div class="flex flex-col items-center gap-1 text-slate-300">
