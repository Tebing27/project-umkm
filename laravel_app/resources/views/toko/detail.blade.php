    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-32">
                ...
                <div class="flex-1 space-y-4 w-full">
                    <div>
                        ...
                        <div class="flex flex-wrap items-center gap-3 mb-2">
                            ...
                            <span
                                class="px-3 py-1 rounded-md bg-[#FFC107] text-slate-900 text-xs font-medium">Kuliner</span>
                        </div>
                    ...
                    <div class="border-t border-slate-200/60 pt-3 grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-4">
                        <div class="flex items-center gap-2">
                            <x-icons.instagram class="w-5 h-5 text-pink-600" />
                            <span class="text-sm text-slate-600">@tebingtsaaa</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <x-icons.facebook class="w-5 h-5 text-blue-600" />
                            <span class="text-sm text-slate-600">tebingtsaaa</span>
                        </div>
                        <div class="flex items-center gap-2">
                        ...
                        <div class="flex items-center gap-2">
                            <x-icons.globe class="w-5 h-5 text-slate-600" />
                            <span class="text-sm text-slate-600">google.com</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <x-icons.whatsapp class="w-5 h-5 text-green-500" /></a>
                        </div>
                    </div>
                    ...
                    <div class="pt-2">
                        <button
                            class="bg-[#FFC107] hover:bg-yellow-400 text-slate-900 font-medium px-6 py-2.5 rounded-lg shadow-sm transition-all active:scale-95 text-sm">
                            Lihat Lokasi
                        </button>
                    </div>
                </div>
        ...
        <div x-data="productLogic()">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                ...
                {{-- Wrapper Filter & Search (Responsive Column di Mobile, Row di Desktop) --}}
                <div class="flex flex-row sm:flex-row gap-3 w-full md:w-auto">
                    ...
                    <div class="relative w-64 sm:w-64">
                        ...
                        <input type="text" x-model.debounce.300ms="search" placeholder="Cari menu favorit..."
                            class="pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none w-full bg-slate-50 focus:bg-white transition-all shadow-sm">
                    </div>
                    {{-- Dropdown Filter Button Wrapper --}}
                    <div class="relative" @click.outside="filterOpen = false">
                        <button @click="filterOpen = !filterOpen"
                            :class="category !== 'Semua' ? 'bg-[#FFC107] text-slate-900' :
                        ...
                        <div x-show="filterOpen" x-transition.origin.top.right x-cloak
                            ...
                            <button @click="setCategory('Semua')"
                                class="w-full text-left px-4 py-2 text-sm hover:bg-slate-50 flex justify-between items-center"
                                :class="category === 'Semua' ? 'text-primary font-bold' : 'text-slate-600'">
                                Semua
                                <span x-show="category === 'Semua'" class="text-primary">✓</span>
                            </button>
                            <button @click="setCategory('Makanan')"
                                class="w-full text-left px-4 py-2 text-sm hover:bg-slate-50 flex justify-between items-center"
                                :class="category === 'Makanan' ? 'text-primary font-bold' : 'text-slate-600'">
                                Makanan
                                <span x-show="category === 'Makanan'" class="text-primary">✓</span>
                            </button>
                            <button @click="setCategory('Minuman')"
                                class="w-full text-left px-4 py-2 text-sm hover:bg-slate-50 flex justify-between items-center"
                                :class="category === 'Minuman' ? 'text-primary font-bold' : 'text-slate-600'">
                                Minuman
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <template x-for="product in displayedProducts" :key="product.id">
                    {{-- Card Wrapper: Hapus 'p-3', tambah 'overflow-hidden' --}}
                    <div
                        ...
                        {{-- Text Content Wrapper: Area baru dengan padding --}}
                        <div class="p-5 flex flex-col flex-1 justify-between">
                            ...
                            <div class="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
                                <div>
                                    <p class="text-[12px] text-slate-400 font-bold tracking-wider mb-0.5">
                                        Harga</p>
                ...
                <div x-show="filteredProducts.length === 0" x-cloak class="col-span-full text-center py-12">
                    <div class="bg-slate-50 rounded-2xl p-8 border border-slate-100 inline-block">
                        <p class="text-slate-600 mb-2">Produk "<span x-text="search" class="font-bold"></span>" tidak
                            ditemukan.</p>
                        <button @click="resetAll" class="text-primary font-bold hover:underline">Reset Semua
                            Filter</button>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 text-center" x-show="hasMore" x-cloak>
                <button @click="loadMore"
                    class="group flex items-center gap-2 mx-auto px-6 py-2.5 rounded-full border border-slate-200 text-slate-600 font-bold text-sm hover:border-primary hover:text-primary hover:bg-slate-50 transition-all duration-300 shadow-sm hover:shadow-md">
                    <span>Lihat Lebih Banyak</span>
                    <x-icons.arrow-down class="w-5 h-5 group-hover:translate-y-0.5 transition-transform" />
                </button>
            </div>
        </div>
