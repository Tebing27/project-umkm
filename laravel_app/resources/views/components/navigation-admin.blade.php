    ...
    <nav class="flex-1 px-4 space-y-2 mt-2 overflow-y-auto overflow-x-hidden flex flex-col scrollbar-hide"
        ...
        @foreach ($menus as $menu)
            <a href="{{ $menu['url'] }}"
                ...
                {{-- Icon --}}
                {{-- CATATAN: Ini tetap pakai titik dua (:) karena isinya PHP (request()->is...), bukan variabel JS --}}
                <x-dynamic-component :component="'icons.' . $menu['icon']" :class="'w-6 h-6 shrink-0 transition-transform duration-300 relative z-10 ' .
                    (request()->is(ltrim($menu['url'], '/')) || request()->is(ltrim($menu['url'], '/') . '/*')
                        ? 'scale-110'
                ...
                <span class="font-semibold text-[15px] whitespace-nowrap transition-all duration-300 relative z-10"
                    x-bind:class="sidebarExpanded ? 'opacity-100 translate-x-0' : 'lg:opacity-0 lg:hidden opacity-100'">
